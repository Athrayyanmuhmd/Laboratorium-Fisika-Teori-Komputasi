...
@section('title', 'Edit Alat - ' . $equipment->name)
...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Edit Alat Laboratorium</h1>
                <p class="text-gray-600 mt-1">{{ $equipment->name }} ({{ $equipment->code }})</p>
            </div>
        ...
        <form method="POST" action="{{ route('admin.equipment.update', $equipment) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <select name="category" id="category" required
                        ...
                        <option value="komputer" {{ old('category', $equipment->category) === 'komputer' ? 'selected' : '' }}>Komputer</option>
                        <option value="software" {{ old('category', $equipment->category) === 'software' ? 'selected' : '' }}>Software</option>
                        <option value="elektronik" {{ old('category', $equipment->category) === 'elektronik' ? 'selected' : '' }}>Elektronik</option>
                        <option value="optik" {{ old('category', $equipment->category) === 'optik' ? 'selected' : '' }}>Optik</option>
                        <option value="mekanik" {{ old('category', $equipment->category) === 'mekanik' ? 'selected' : '' }}>Mekanik</option>
                ...
                <div>
                    ...
                    @error('quantity')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                    <p class="text-sm text-gray-500 mt-1">Tersedia: {{ $equipment->available_quantity }} unit</p>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description" 
                          id="description"
                          rows="3"
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 @error('description') border-red-300 @enderror">{{ old('description', $equipment->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                    <div>
                        ...
                        <input type="number" 
                               name="purchase_year" 
                               id="purchase_year"
                               value="{{ old('purchase_year', $equipment->purchase_year) }}"
                               min="1900"
                               max="{{ date('Y') }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="date" 
                               ...
                               id="last_calibration"
                               value="{{ old('last_calibration', $equipment->last_calibration?->format('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <input type="date" 
                               ...
                               id="next_calibration"
                               value="{{ old('next_calibration', $equipment->next_calibration?->format('Y-m-d')) }}"
            ...
            <div class="border-t pt-6">
                <h3 class="text-lg font-medium text-gray-900 mb-4">Gambar Saat Ini</h3>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    @foreach($equipment->images as $image)
                        <div class="relative">
                                                            <img src="{{ asset('storage/' . $image) }}" alt="Equipment Image" class="w-full h-24 object-cover rounded-lg" onerror="this.style.display='none'">
                            <div class="absolute top-1 right-1">
                                <label class="inline-flex items-center">
                                    <input type="checkbox" name="remove_images[]" value="{{ $image }}" class="form-checkbox h-4 w-4 text-red-600">
                                    <span class="ml-1 text-xs text-white bg-red-600 px-1 rounded">Hapus</span>
                                </label>
                            </div>
                        </div>
                    @endforeach
            ...
            <div class="border-t pt-6">
                <div>
                    ...
                    <textarea name="notes" 
                              id="notes"
                              rows="3"
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 @error('notes') border-red-300 @enderror">{{ old('notes', $equipment->notes) }}</textarea>
                    @error('notes')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
